...
(module $lwt
  ...
  (type $itask (func (param i32)))
  (type $itaskc (cont $itask))
  ...
  (func $main-task
    (call $spawn (cont.bind $itaskc $taskc (i32.const 0) (cont.new $itaskc (ref.func $task))))
    (call $spawn (cont.bind $itaskc $taskc (i32.const 1) (cont.new $itaskc (ref.func $task))))
    (call $spawn (cont.bind $itaskc $taskc (i32.const 2) (cont.new $itaskc (ref.func $task))))
    (call $spawn (cont.bind $itaskc $taskc (i32.const 3) (cont.new $itaskc (ref.func $task)))))
  (func $main (export "main")
    (call $bfs (cont.new $taskc (ref.func $main-task))))
  (elem declare func $task $main-task)
)
(register "lwt")
